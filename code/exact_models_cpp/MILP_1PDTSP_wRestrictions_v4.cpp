...
MILP_1PDTSP::MILP_1PDTSP(vector<Node> vNodes, int Q)
{
	try
	{
        ...
        GRBVar sv[numberNodes];
        GRBVar e[numberNodes];
        GRBVar xu, xl;
		...
		for (i = 0; i < numberNodes; i++) {
            ...
            // Depronto puede ser continua y de un valor entero
            string s4 = "E_" + itor(i);
            e[i] = model.addVar((0.0, GRB_INFINITY, 0.0, GRB_CONTINUOUS, s4);
		}
       ...
       // 20. Low demands wont be taken into account
       for (i = 0; i < numberNodes; i++) {
            GRBLinExpr expr = 0;
			for (j = 0; j < numberNodes; j++) {
				expr += x[i][j];
            ...
            string s = "LeftBikesAtEverStat_" + itor(i);
            model.addConstr(expr, GRB_EQUAL, e[i], s);
		}
		...
		// 21. Demands must be higher than rho
        for (i = 0; i < numberNodes; i++) {
            GRBLinExpr expr = 0;
            expr += e[i];
            expr -= vNodes.at(i).getq();;
            string s = "Ei_q_i__" + itor(i) + "_" + itor(j);
            model.addConstr(expr, GRB_LESS_EQUAL, rho, s);
            string s = "Ei_q_i__" + itor(i) + "_" + itor(j);
            model.addConstr(expr, GRB_GREATER_EQUAL, -rho, s);
		}
